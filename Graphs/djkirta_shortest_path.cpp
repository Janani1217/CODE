...
void djkistra(unordered_map<int, list<pair<int, int>>> adj, set<pair<int, int>> &s, vector<int> &dist, int src)
{
    while (!s.empty())
    {
        ...
        for (auto neigh : adj[node])
        {
            if (dist[neigh.first] > neigh.second + node_dist)
            {
                ...
                // It could have been vis by some other node and inserted into the set previosly
